...
public class RealtyObjectCSVHelper
{
    public static void ExportToCSV(List<RealtyObject> realtyObjects, string filePath)
    {
        ...
        {
            string type = obj is PropertyQuadObject ? "Property" : "Parcel";
            string number = obj is PropertyQuadObject p ? p.PropertyNumber.ToString() : ((ParcelQuadObject)obj).ParcelNumber.ToString();
            ...
            string description = obj is PropertyQuadObject ? ((PropertyQuadObject)obj).Description : ((ParcelQuadObject)obj).Description;
    ...
    public static List<RealtyObject> ImportFromCSV(string filePath)
    {
        ...
        foreach (var line in lines.Skip(1))
        {
            ...
            {
                var property = new PropertyQuadObject(
                    int.Parse(fields[1]),
            ...
            {
                var parcel = new ParcelQuadObject(
                    int.Parse(fields[1]),
                    fields[2].Trim('"'), // Assuming description is enclosed in quotes
                    new GPSRectangle(
                        new GPSPoint(lowerLeftLatDirection,
